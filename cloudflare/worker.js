/**
 * Cloudflare Worker 独立版本 - 抖音短视频/图集解析与代理下载
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/health" || url.pathname === "/api/health") {
      return jsonResponse({
        status: "ok",
        service: "douyin-downloader-worker",
    ...
    if (url.pathname === "/api/parse" && request.method === "POST") {
      try {
        ...
        return jsonResponse({ success: false, error: err.message || "服务器内部错误" }, 500);
      }
    }
    ...
    if (url.pathname === "/api/download" && request.method === "GET") {
      ...
      try {
        ...
        return new Response(`下载代理错误: ${err.message}`, { status: 500, headers: corsHeaders });
      }
    }
